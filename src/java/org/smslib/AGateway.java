...
public abstract class AGateway
{
	...
	public AGateway(String id)
	{
		...
		this.gatewayStatus = GatewayStatuses.STOPPED;
	...
	{
		if (getStatusNotification() != null) getStatusNotification().process(getGatewayId(), getGatewayStatus(), status);
		if (getService().getGatewayStatusNotification() != null) getService().getGatewayStatusNotification().process(getGatewayId(), getGatewayStatus(), status);
	...
	public void startGateway() throws TimeoutException, GatewayException, IOException, InterruptedException
	{
		...
		setGatewayStatus(GatewayStatuses.RUNNING);
	}

	@SuppressWarnings("unused")
	public void stopGateway() throws TimeoutException, GatewayException, IOException, InterruptedException
	{
		setGatewayStatus(GatewayStatuses.STOPPED);
	...
	private class QueueManager implements Runnable
	{
		...
		public void run()
		{
			...
			try
			{
				if (getGatewayStatus() == GatewayStatuses.RUNNING)
				{
					while (true)
					{
						msg = AGateway.this.messageQueue.take();
						if (getGatewayStatus() != GatewayStatuses.RUNNING) break;
						if (msg != null)
						{
							if (!sendMessage(msg))
							{
			...
			catch (Exception e)
			{
				...
				setGatewayStatus(GatewayStatuses.RESTART);
				try
				{
					if ((msg != null) && (msg.getMessageStatus() != MessageStatuses.SENT)) queueMessage(msg);
				}
