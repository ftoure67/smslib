...
public class Router
{
	...
	protected void preroute(OutboundMessage msg)
	{
		for (AGateway gtw : getService().getGateways())
			if ((gtw.isOutbound()) && (gtw.getGatewayStatus() == GatewayStatuses.RUNNING)) getCandidates().add(gtw);
	}
	...
	{
		if ((msg.getGatewayId().length() == 0) || (msg.getGatewayId().equals("*"))) getAllowed().addAll(getCandidates());
		else
		{
			if (getService().findGateway(msg.getGatewayId()) != null) getAllowed().add(getService().findGateway(msg.getGatewayId()));
		}
	}
