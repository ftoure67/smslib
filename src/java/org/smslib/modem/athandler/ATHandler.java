...
public class ATHandler extends AATHandler
{
	...
	public boolean setIndications() throws TimeoutException, GatewayException, IOException, InterruptedException
	{
		while (true)
		{
			...
			try
			{
				CNMIDetector c = new CNMIDetector(getModemDriver().getResponse(), getGateway());
				getModemDriver().write(c.getATCommand());
				getModemDriver().getResponse();
				return (getModemDriver().isOk());
			}
			catch (Exception e)
			{
				getGateway().getService().getLogger().logWarn("Retrying the detection of CNMI, modem busy?", e, getGateway().getGatewayId());
