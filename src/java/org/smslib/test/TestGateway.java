...
public class TestGateway extends AGateway
{
	...
	private InboundMessage generateIncomingMessage()
	{
		...
		return new InboundMessage(new java.util.Date(), "+1234567890", "Hello World! #" + getInboundMessageCount(), 0, null);
	}
		...
		this.incomingMessagesThread = new Thread(new Runnable()
		{
			...
			public void run()
			{
				while (!TestGateway.this.incomingMessagesThread.isInterrupted())
				{
					...
					if (!TestGateway.this.incomingMessagesThread.isInterrupted() && ((getInboundNotification() != null) || (getService().getInboundNotification() != null)))
					{
						getService().getLogger().logInfo("Detecting incoming message", null, getGatewayId());
						if (getInboundNotification() != null ) getInboundNotification().process(getGatewayId(), MessageTypes.INBOUND, generateIncomingMessage());
	...
	public boolean sendMessage(OutboundMessage msg) throws TimeoutException, GatewayException, IOException, InterruptedException
	{
		...
		getService().getLogger().logInfo("Send to: " + msg.getRecipient() + " via: " + msg.getGatewayId(), null, getGatewayId());
		return true;
	}
