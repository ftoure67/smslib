...
public class SMSServer implements Runnable
{
	...
	List<AInterface<? extends Object>> infList;
	...
	public SMSServer()
	{
		...
		this.infList = new ArrayList<AInterface<? extends Object>>();
	...
	public List<AInterface<? extends Object>> getInfList()
	{
		return this.infList;
	}
...
//			throw new SMSLibException("Cannot find SMSServer configuration file!");
		...
		for (int i = 0; i < Integer.MAX_VALUE; i++)
		{
			try
			{
				...
				Constructor<?> constructor = c.getConstructor(argsClass);
				AInterface<? extends Object> inf = (AInterface<? extends Object>) constructor.newInstance(args);
				getInfList().add(inf);
	...
	{
		for (AInterface<? extends Object> inf : getInfList())
			inf.start();
	}
